...
Github: https://github.com/zhuxiaoyu123/ribosomal_proteins_tree
...
1. The sequence names must be composed of genome name and gene id with "__" as separator:
   >MAG6666__gene001
   >MAG7777__gene002
   ...
   which is a good habit and is benefit for downstream analyses.
...
system "touch $outdir/COGxxxx_temp.faa";  #不知道为什么最后一个temp.faa总是无法处理，所以创了一个临时的空文件，等循环跑完再删除
foreach my $genes (@selected_genes) {
	my %hash;
